...
function __powerline_python_venv_prompt {
  ...
  [[ -n "${python_venv}" ]] && echo "${PYTHON_VENV_CHAR}${python_venv}|${PYTHON_VENV_THEME_PROMPT_FG_COLOR}|${PYTHON_VENV_THEME_PROMPT_COLOR}"
}
...
function __powerline_scm_prompt {
  ...
  if [[ "${SCM_NONE_CHAR}" != "${SCM_CHAR}" ]]; then
    ...
    echo "${scm_prompt}${scm}|${SCM_THEME_PROMPT_FG_COLOR}|${color}"
  fi
}
...
function __truncate_cwd {
  ...
  if [ $(echo -n $newPWD | wc -c | tr -d " ") -gt $CWD_MAX_LENGTH ]
  then
     newPWD=$(echo -n $newPWD | awk -F '/' '{
     print $1 "/$CWD_SYMBOL/" $(NF-1) "/" $(NF)}')
  fi
  echo $newPWD
}

#function __powerline_cwd_prompt {
#  echo "$(__truncate_cwd | sed "s|^${HOME}|~|")|$CWD_THEME_PROMPT_FG_COLOR|${CWD_THEME_PROMPT_COLOR}"
#}

function __powerline_cwd_prompt {
  echo "$(__truncate_cwd)|$CWD_THEME_PROMPT_FG_COLOR|${CWD_THEME_PROMPT_COLOR}"
}
...
function __powerline_left_segment {
  ...
  if [[ "${SEGMENTS_AT_LEFT}" -gt 0 ]]; then
    separator="$(set_color ${LAST_SEGMENT_COLOR} ${params[2]})${separator_char}${normal}"
  fi
  LEFT_PROMPT+="${separator}$(set_color ${params[1]} ${params[2]}) ${params[0]} ${normal}"
  LAST_SEGMENT_COLOR=${params[2]}
  (( SEGMENTS_AT_LEFT += 1 ))
}
